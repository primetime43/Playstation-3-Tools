...
namespace PS3Util
{
    class PS3
    {
        ...
        {
            string HexPointer = BitConverter.ToString(PS3.ReadBytes(addr, 4)).Replace("-", "");
            int Address = Convert.ToInt32(HexPointer, 16);
            return (uint)Address;
        }
        public static uint PointToDvar(uint addr)
        {
            return FollowPointer(addr) + 0x18;
        }
